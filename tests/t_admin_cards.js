...
ok([...r.d.querySelectorAll('#accountLifecycleRoot .workspace-table th')].map(cell=>cell.textContent).join(',')==='ID,Name,Role,Created at,Status,History','lifecycle table places the account creation year between role and status');
...
ok(/\.lifecycle-popover\s*\{[^}]*text-align:\s*left;/.test(lifecycleCss)&&/\.lifecycle-popover label\s*\{[^}]*justify-content:\s*flex-start;[^}]*text-align:\s*left;/.test(lifecycleCss),'lifecycle navigation prompts align their text to the left');
...
lifecycleTools.querySelector('[aria-label="Group by"]').click();lifecycleTools.querySelector('input[name="lifecycleGroup"][value="Students"]').click();
ok(lifecycleTools.querySelectorAll('.workspace-group-row').length>0,'grouping inserts role-group labels');
...
r.d.querySelector('.lifecycle-profile [data-profile-tab="history"]').click();
ok([...r.d.querySelectorAll('.lifecycle-profile .workspace-table th')].map(cell=>cell.textContent).join(',')==='Time and date,What changed,Who changed','profile History tab shows change time, details, and editor');
ok(!!r.d.querySelector('.lifecycle-profile [aria-label="Search history"]')&&!!r.d.querySelector('.lifecycle-profile [aria-label="Filter by editor"]')&&!!r.d.querySelector('.lifecycle-profile [aria-label="Group history"]'),'history tab has search, editor filter, order, and grouping controls');
