...
const w=dom.window,data={systemSettings:{},adminAnnouncements:[],subjects:[{code:'OLD',name:'Old catalog subject'}],curricula:[{program:'BSCS',yearLevel:1,term:'first',subjectCode:'SUB1',subjectName:'HCI',units:3,status:'active'}],sections:[{id:'1BSCS-1',program:'BSCS',yearLevel:1,capacity:50}],students:[{id:'S1',sections:['1BSCS-1']}],sectionSubjects:[{sectionId:'1BSCS-1',assignments:[{id:'OLD-OFFER',subjectCode:'OLD',facultyId:'F1'}]}],studentEnrollments:[{id:'OLD-ENROLLMENT',studentId:'S1',sectionId:'1BSCS-1',subjectCode:'OLD',offeringId:'OLD-OFFER'}]};
...
ok(data.subjects.some(item=>item.code==='SUB1'&&item.name==='HCI'&&item.units===3),'Load Management derives the active subject catalog');
ok(data.sectionSubjects[0].assignments.map(item=>item.subjectCode).join('|')==='SUB1','Load Management derives section offerings');
ok(data.studentEnrollments.map(item=>item.subjectCode).join('|')==='SUB1','Load Management derives current-semester student enrollment');

const toolbar=w.document.querySelector('.curriculum-admin-toolbar');
ok(!!toolbar,'curriculum toolbar renders');
const subjectRows=[...w.document.querySelectorAll('.curriculum-subject-row')];
ok(subjectRows.length>0&&subjectRows.every(row=>row.querySelectorAll('.curriculum-drag-handle').length===1),'every subject has one drag handle');
ok([...w.document.querySelectorAll('.semester-heading-copy>span')].every(meta=>!meta.textContent.includes('curriculum units')),'semester subtitle omits the redundant curriculum-unit total');
...
w.SystemManagement.curriculumToolbarAction('reorder');
ok(!w.document.getElementById('curriculumEditorModal'),'Reorder uses drag handles instead of a pop-out form');
...
const generatedCode=w.document.querySelector('.curriculum-editor-modal input[name="subjectCode"]');
ok(generatedCode?.value==='CICS1101'&&generatedCode.readOnly,'Add subject automatically provides a read-only CICS code');
data.subjects.push({code:'CICS1101',name:'Existing CICS subject'});
w.SystemManagement.openMergedSubjectEditor();
ok(w.document.querySelector('.curriculum-editor-modal input[name="subjectCode"]')?.value==='CICS1102','automatic CICS codes advance past existing values');
